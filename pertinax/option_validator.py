...
class OptionValidator(object):
    ...
    def exists(self, opt_dest):
        ...
        if isinstance(self.options, dict):
            return (opt_dest in self.options) and (self.options.get(opt_dest) is not None)
        else:
            return (getattr(self.options, opt_dest) is not None)
        ...
        for opt_dest in opt_dests:
            if not self.exists(opt_dest):
                ...
                if message:
                    ...
                    self.add_option_error(_('Option %s is required') % flag)
    ...
    def reject(self, opt_dest, colliding_opts=None, message=None):
        ...
        if colisions:
            ...
            if message:
                ...
                if len(colisions) > 1:
                    self.add_option_error(_('Options %(flags)s are colliding with %(coll_f)s') % \
                        {'flags':flags, 'coll_f':colliding_flags})
                else:
                    self.add_option_error(_('Option %(flags)s is colliding with %(coll_f)s') % \
                        {'flags':flags, 'coll_f':colliding_flags})
            else:
                if len(colisions) > 1:
                    self.add_option_error(_('Options %s can\'t be used in this command') % flags)
                else:
                    self.add_option_error(_('Option %s can\'t be used in this command') % flags)
    ...
    def require_one_of(self, opt_dests, message=None):
        ...
        if not len(self.__filter_existing(opt_dests)) == 1:
            if message:
                ...
                self.add_option_error(_('Exactly one of %s is required') % ', '.join(flags))
        return
    ...
    def require_at_least_one_of(self, opt_dests, message=None):
        ...
        if message:
            ...
            flags = self.__get_option_strings(opt_dests)
            self.add_option_error(_('At least one of %s is required') % ', '.join(flags))
